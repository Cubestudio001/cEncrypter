...
stdens="$infile:<red ascii>$.:dt$,:sq$/:lr$\:rl$;:ff$':sg$\":db"
    ...
    if action == '加密':
        ...
        while True:
            ...
            if _createf == '是':
                ...
                continue
        if dc_type == 'doc': 
            ecode = ensreader.encode(operate,std,method='l',createfile=_createf)
        elif dc_type == 'str':
            ecode = ensreader.encode_str(operate,std,method='l',createfile=_createf)
        '''
        if _createf:
            while True:
